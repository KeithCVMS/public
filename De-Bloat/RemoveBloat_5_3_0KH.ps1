...
#    "C27EB4BA.DropboxOEM*"															##kh wildcards wont work	
    "C27EB4BA.DropboxOEM"																##kh removed trailing *
...
##If $custombloatist is set, split on the comma and add to bloatlist
if ($custombloatlist) {
...
if ($version -like "*Windows 10*") {
    Log "Removing Windows Backup from Win10"
    ...
    if (Test-Path $filepath) {
        ...
        Log "Adding Windows Shell Components to Win10"
...
foreach ($32app in $32apps) {
	write-output "   - $($32app.DisplayName)"	#KH added for logging
...
foreach ($64app in $64apps) {
	write-output "   - $($64app.DisplayName)"	#KH added for logging
    #Get uninstall string
...
if ([System.Security.Principal.WindowsIdentity]::GetCurrent().Name -ne "NT AUTHORITY\SYSTEM") {
		...
		if (Test-Path $path1) {
				...
				foreach ($32app in $32apps) {
		write-output "   - $($32app.DisplayName)"	#KH added for loggin
						#Get uninstall string
						$string1 = $32app.uninstallstring
						#Check if it's an MSI install
						if ($string1 -match "^\s*(C:\\Windows\\System32\\)?msiexec(\.exe)?\s+\S*") {
...
if ($IsOOBEComplete -eq 0) {
	...
	## The XML below will Remove Retail Copies of Office 365 and OneNote, including all languages. 
	## Note: Office Apps for Entreprise Editions will remain.
